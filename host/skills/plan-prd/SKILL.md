---
description: Plan a feature and create a PRD item for ralph-once
---

# Plan to PRD

Guide a planning session and create a comprehensive PRD item. PRD items are consumed by ralph-once which runs Claude autonomously to implement features.
...
## Writing for ralph-once Automation

PRD items are consumed by ralph-once which runs Claude autonomously to implement features.
The PRD must be self-contained and unambiguous for automated implementation.

### What ralph-once expects:
- Picks highest priority feature with `passes: false`
- Implements following the steps exactly
- Runs feedback loops (as defined in the plan's CLAUDE.md)
...
1. **Required packages** - If the feature needs any packages not already in package.json:
   ...
   - ralph-once will BLOCK if it needs a package not specified here
...
6. **Verification steps** that match the plan's feedback loop expectations
